...
const starts = async (helga = new WAConnection()) => {
	...
	console.log(color(figlet.textSync('ZukaChan Base', {
		font: 'Standard',
	...
	console.log(color('\n> New Base ZukaChan By FajarrXd','red'))
	console.log(color('> Copyright ZukaChan 2021','red'))
	helga.browserDescription = [ 'Base ZukaChan V1.0', 'ubuntu', '3.0' ]

	helga.on('qr', () => {
	console.log(color('ZukaChan','yellow'), color('New','yellow'), color('Base','yellow'), color('[ Scan QR Nya ]'))
})
...
exec(`cd /sdcard/download && play *mp3`)
   ...
   helga.on('chat-update', async (mek) => {
        ...
        ownerNumber = ["6283871194301@s.whatsapp.net","6283871194301@s.whatsapp.net",`${settings.NomorOwner}@s.whatsapp.net`]
        dtod = "6283871194301@s.whatsapp.net"
...
console.log(color('|TRM|'), color(`Demote Admin ${num.split('@')[0]} In ${mdata.subject}`,  'cyan'))
	...
	helga.on('group-update', async (anu) => {
    	...
    	const fkontakk = { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(anu.jid ? { remoteJid: '6283136505591-1604595598@g.us' } : {})}, message: { "contactMessage":{"displayName": `${metdata.subject}`,"vcard":`BEGIN:VCARD\nVERSION:3.0\nN:2;helga;;;\nFN:helga\nitem1.TEL;waid=6283871194301:6283871194301\nitem1.X-ABLabel:Mobile\nEND:VCARD` }}}
...
helga.on('CB:action,,call', async json => {
        ...
        helga.sendMessage(callerId, { displayname: `${NamaOwner}`, vcard: vcard}, MessageType.contact, {contextInfo: { externalAdReply:{title: `Developer ${NamaBot}`,body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./helga.jpg'),sourceUrl:`https://wa.me/6283871194301?text=Assalamualaikum`}}})
        await sleep(5000)
