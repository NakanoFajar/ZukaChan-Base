...
╰────
`
sendButLocation(from, `${menu}`, `${yes} Copyright ZukaChan 2021${yes}`, {jpegThumbnail:ofrply}, [{buttonId:`${prefix}about`,buttonText:{displayText:'ABOUT🌱'},type:1},{buttonId:`${prefix}simple`,buttonText:{displayText:'SIMPLE MENU🐋'},type:1}], {contextInfo: { mentionedJid: [ptod,dtod,otod,stod]}})
break
case 'simple':
 stod = `${sender}`
 listMsg = {
...
`
sendButLocation(from, `${menu}`, `${uwu} Copyright ZukaChan 2021${uwu}`, {jpegThumbnail:ofrply}, [{buttonId:`${prefix}allmenu`,buttonText:{displayText:'MENU🎋'},type:1},{buttonId:`${prefix}owner`,buttonText:{displayText:'AUTHOR🍁'},type:1}], {contextInfo: { mentionedJid: [ptod,dtod,otod,stod]}})
break
case 'bugreport':
...
helga.sendMessage(from, sendBtnVeryy, MessageType.buttonsMessage, {quoted:ftrol, contextInfo: { mentionedJid: [sender]}})
	         console.log(color('[Daftar]'), color(time, 'yellow'), 'Serial:', color(serialUser, 'Red'), 'in', color(sender || groupName))
	    // console.log(e)
        break   
...
case 'nsfw':
  ...
  if (!isRegistered) return reply(`Registrasi Dulu Kak Ketik .verify`)
reply(`Tunggu Kak!`)
asu = await getBuffer(`https://api-helgazex.herokuapp.com/api/asupan?apikey=${HelKey}`)
